...
class TradingStrategy(Enum):
    ...
    BOTH = "both"
    SMART_LIMIT = "smart_limit"  # 新增智能限价单策略
...
class TradingPairConfig:
    ...
    min_profitable_spread: float = 0.001  # 新增最小盈利价差
    min_depth_quality: float = 1.5  # 新增最小深度质量
...
class SmartMarketMaker:
    def __init__(self):
        ...
        # 策略选择
        strategy_str = os.getenv('TRADING_STRATEGY', 'SMART_LIMIT').upper()
        self.strategy = getattr(TradingStrategy, strategy_str, TradingStrategy.SMART_LIMIT)
    ...
    def load_trading_pairs_config(self) -> List[TradingPairConfig]:
        ...
        for pair_symbol in pairs_list:
            ...
            min_profitable_spread = float(os.getenv(f'{base_asset}_MIN_PROFITABLE_SPREAD', 0.001))
            min_depth_quality = float(os.getenv(f'{base_asset}_MIN_DEPTH_QUALITY', 1.5))
            ...
            self.logger.info(f"   最小盈利价差: {min_profitable_spread:.4%}")
            self.logger.info(f"   最小深度质量: {min_depth_quality}")
    ...
    def check_market_conditions_optimized(self, pair: TradingPairConfig) -> bool:
        ...
        # 增加额外条件：只有在价差足够小时才交易
        min_profitable_spread = pair.min_profitable_spread  # 考虑手续费后的最小盈利价差
        if spread < min_profitable_spread:
            self.logger.info(f"{pair.symbol}价差过小({spread:.4%})，不足以覆盖手续费，跳过交易")
            return False
        
        # 检查市场深度质量
        min_depth_quality = pair.min_depth_quality  # 买卖深度比例
    ...
    def strategy_smart_limit(self, pair: TradingPairConfig) -> bool:
        ...
        try:
            ...
            sell_quantity, _ = self.get_sell_quantity(pair, sell_client_name)
            # 买单数量：动态计算
            buy_quantity = self.calculate_dynamic_quantity(pair)
    ...
    def execute_trading_cycle_optimized(self, pair: TradingPairConfig) -> bool:
        """优化的交易周期执行"""
        ...
        # 优先使用智能限价单策略（磨损最小）
        if self.strategy in [TradingStrategy.BOTH, TradingStrategy.LIMIT_MARKET, TradingStrategy.SMART_LIMIT]:
    ...
    def execute_trading_cycle(self, pair: TradingPairConfig) -> bool:
        """执行一个交易周期（兼容旧接口）"""
        return self.execute_trading_cycle_optimized(pair)
    ...
    def print_trading_statistics(self):
        ...
        for pair in self.trading_pairs:
            ...
            self.logger.info(f"     动态交易量使用次数: {state.get('dynamic_quantity_used', 0)}")
    ...
    def monitor_and_trade(self):
        ...
        while self.is_running:
            try:
                ...
                # 执行交易
                if self.execute_trading_cycle_optimized(current_pair):
                    consecutive_failures = 0
                    # 每5次成功交易打印一次余额和统计
